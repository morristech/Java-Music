...
public class Pitch implements Comparable<Pitch> {
    ...
    public Pitch transpose(Interval interval) {
        ...
        if (addedPitchClass.getPitchLetter().compareTo(this.pitchClass.getPitchLetter()) <= 0) {
            // Since it's checking for less/equals to the current PitchLetter, don't increment the octave if
            // the pitch is transposed by a unison interval (which yields equals to current PitchLetter)
            if (interval.getNumberSpan() != 0) {
                addedOctave++;
            }
        }
